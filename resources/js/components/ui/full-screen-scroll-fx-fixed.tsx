...
export const FullScreenScrollFXFixed = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    {
      sections,
      className,
    ...
    useLayoutEffect(() => {
      ...
      if (bgRefs.current[initialIndex]) {
        gsap.set(bgRefs.current[initialIndex], { opacity: 1, scale: 1 });
      }

      wordRefs.current.forEach((words, sIdx) => {
        words.forEach((w) => {
          gsap.set(w, {
            yPercent: sIdx === initialIndex ? 0 : 100,
            opacity: sIdx === initialIndex ? 1 : 0,
          });
        });
      });
    ...
    return (
      ...
      >
        ...
        <style>{`
          ...
          .fx-fixed-section { height: ${total * 100}vh; position: relative; }
...
FullScreenScrollFXFixed.displayName = "FullScreenScrollFXFixed";
