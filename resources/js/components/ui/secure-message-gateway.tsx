...
import { Terminal, PlusIcon, MessageCircle } from "lucide-react";
...
export const SecureMessageGateway = () => {
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        const phoneNumber = "254718574798";
        const text = encodeURIComponent(message || "Hello Nderu.ke, I'm reaching out from your website.");
    ...
    return (
        <div className="flex items-center justify-center min-h-[300px] w-full p-4 bg-background">
            <div className="relative w-full max-w-2xl bg-transparent border border-border border-dashed shadow-sm p-6 sm:p-10 transition-all rounded-none">
                ...
                <CreateCornors>
                    <PlusIcon className="font-[200] text-muted-foreground" />
                </CreateCornors>
      ...
      `,
                ...
                <div className="backdrop-blur-xs p-2 rounded-xs">
                    ...
                    <div className="mb-6 z-10">
                        <h2 className="text-[0.6rem] font-bold uppercase tracking-[0.2em] text-primary mb-1 flex border-b border-b-border pb-2 items-center gap-2">
                            <div className="size-1.5 bg-foreground rounded-full animate-pulse" />
                            <span className="text-foreground">Nderu.ke</span>
                        </h2>
                        <h1 className="text-xl font-semibold text-foreground tracking-tight">
                            Reach out to <span className="text-foreground">Nderu.ke </span>
                        </h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex sm:flex-row items-stretch gap-2">
                        ...
                        <div className="relative flex-1 group">
                            ...
                            <input
                                ...
                                className={cn(
                                    ...
                                    "focus:bg-accent focus:ring-1 focus:ring-primary/20 focus:border-primary border-dashed",
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                !pending && "hover:border-primary hover:text-primary hover:bg-accent active:scale-95",
                                pending && "opacity-40 cursor-not-allowed"
                            )}
                        >
                            <MessageCircle size={14} className={cn(pending && "animate-bounce")} />
                            <span>{pending ? "CONNECTING..." : "WHATSAPP"}</span>
                        </button>
                    </form>
                </div>
                {/* Status Line */}
                <div className="mt-6 flex items-center justify-between">
                    <span className="text-[0.55rem] font-mono uppercase tracking-widest text-muted-foreground">
                        Status: {pending ? <span className="text-muted-foreground">Establishing Secure Link...</span> : <span className="text-foreground border p-0.5 border-border">Encrypted Link Ready</span>}
                    </span>
