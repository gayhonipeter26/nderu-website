...
const PRODUCT_DATA: Record<ProductId, ProductData> = {
  left: {
    ...
    description: 'Full-stack web applications with Vue.js, Laravel, and modern architecture. Building scalable, performant solutions tailored to your business needs with clean code and best practices.',
    image: 'https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=800&h=800&fit=crop&crop=entropy&auto=format&q=75',
    colors: {
      gradient: 'from-blue-600 to-indigo-900',
      glow: 'bg-blue-500',
      ring: 'border-l-blue-500/50',
    },
    stats: { connectionStatus: 'Available Now', batteryLevel: 98 },
    features: [
      { label: 'Performance', value: 95, icon: Code },
      { label: 'Scalability', value: 92, icon: Wifi },
    ],
  },
  right: {
    id: 'right',
    ...
    description: 'Commercial, event, and portrait photography services. High-quality visual content creation with professional editing, retouching, and delivery for brands and individuals.',
    image: 'https://images.unsplash.com/photo-1542038784456-1b839f85bb9b?w=800&h=800&fit=crop&crop=entropy&auto=format&q=75',
    colors: {
      gradient: 'from-emerald-600 to-teal-900',
      glow: 'bg-emerald-500',
      ring: 'border-r-emerald-500/50',
    },
    stats: { connectionStatus: 'Available Now', batteryLevel: 94 },
    features: [
      { label: 'Quality', value: 98, icon: Camera },
      { label: 'Creativity', value: 96, icon: Music },
    ],
  },
};
...
const ProductVisual = ({ data, isLeft }: { data: ProductData; isLeft: boolean }) => (
  <motion.div layout="position" className="relative group shrink-0">
    ...
    <div className="relative h-80 w-80 md:h-[450px] md:w-[450px] rounded-full border border-white/5 shadow-2xl flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-sm">
      ...
      >
        {/* Orbiting Skills will be rendered here */}
        <div className="w-full h-full flex items-center justify-center">
          <OrbitingSkills activeService={isLeft ? 'development' : 'photography'} />
        </div>
      </motion.div>
    </div>
    ...
    >
      ...
      <motion.div variants={ANIMATIONS.item} className="w-full space-y-6 bg-zinc-900/40 p-6 rounded-2xl border border-white/5 backdrop-blur-sm">
        ...
        <div className={`pt-4 flex ${isLeft ? 'justify-start' : 'justify-end'}`}>
          <a 
            href={isLeft ? '/projects' : '/photography'} 
      ...
      <motion.div variants={ANIMATIONS.item} className={`mt-6 flex items-center gap-3 text-zinc-500 ${flexDirClass}`}>
        <Battery size={16} />
        <span className="text-sm font-medium">{data.stats.batteryLevel}% Availability</span>
      </motion.div>
    </motion.div>
  );
};
...
const Switcher = ({ 
  ...
  return (
    <div className="absolute top-8 inset-x-0 flex justify-center z-50 pointer-events-none">
...
export default function SpatialProductShowcase() {
  ...
  return (
    <div className="relative w-full h-screen bg-black text-zinc-100 overflow-hidden selection:bg-zinc-800 flex flex-col items-center justify-center">
